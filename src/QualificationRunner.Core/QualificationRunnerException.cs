...
namespace QualificationRunner.Core
{
   public class QualificationRunnerException : OSPSuiteException
   {
      ...
      public QualificationRunnerException(IReadOnlyList<QualificationRunResult> invalidResults) : base(invalidResults.Select(x => Errors.ProjectConfigurationNotValid(x.Project, x.LogFile)).ToString("\n"))
      {
      }
   }
}